...
public class FollowCam : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // 주인공이 장에물에 가려졌는지를 판단할 레이캐스트의 높낮이를 설정
    ...
    {
        // 카메라의 높이와 거리를 계산. 일정 거리만큼 뒤로 떨어져서 추적 & 전방의 시야를 확보해야 하기 때문에 이렇게 계산함.
        ...
        // 회전할 때의 속도 계수를 적용. 부드럽게 움직이기 위해서 적용
        tr.rotation = Quaternion.Slerp(tr.rotation
                                        , target.rotation
                                        , Time.deltaTime * rotateDamping);

        // 카메라를 추적 대상으로 Z축을 회전시킴. 현재 플레이어의 캐릭터 Pivot이 발에 있으므로 바라보는 방향을 높혀줌
