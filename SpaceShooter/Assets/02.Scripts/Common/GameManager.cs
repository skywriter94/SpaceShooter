...
public class GameManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        if(instance == null) // 처음에 실행하는지 판단해 null이면 GameManager 클래스(스크립트)를 할당.
        {
            instance = this;
        }
        // instance에 할당된 클래스의 인스턴스가 다를 경우 새로 생성된 클래스를 의미함. 다른 씬으로 전환했다가 다른 씬으로 오면 다른 GameManager 스크립트의 Awake 함수가 실행되는데, 이때 instance 변수는 static 변수이기에 값이 들어있고, 따라서 else if 구문을 실행하게 된다. 당연히 처음 생성된 GameManager의 인스턴스와 두 번째 생성된 GameManager의 인스턴스가 다르기 때문에 두 번째 생성된 GameManager 인스턴스는 삭제한다. 즉, 최초에 생성된 GameManager만 남게 되므로 하나의 클래스가 지속해서 유지하는 것이다.
        ...
        // 인벤토리에 추가된 아이템을 검색하기 위해 SlotList 게임오브젝트를 추출
    ...
    IEnumerator CreateEnemy()
    {
        ...
        while(!isGameOver)
        {
            ...
            if(enemyCount < maxEnemy)
            {
                ...
                // 불규칙적인 위치 산출. Parent인 SpawnPointGroup 게임오브젝트 자신도 배열 0번째 추가되어 있어 1번부터 시작함.
