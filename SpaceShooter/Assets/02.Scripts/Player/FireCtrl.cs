...
public class FireCtrl : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // 레이어 마스크의 비트 연산(OR 연산). enemyLayer 와 obstacleLayer 태그 정보를 가진 두 개의 레이어만 검출 한다.
    ...
    void Update()
    {
        ...
        if (EventSystem.current.IsPointerOverGameObject()) return; // UI를 누를 시에 True를 반환. 대신에 Raycast Target이 언체크 되어있으면 감지 못함.
    ...
    {
        // 셰이크 효과 호출
        StartCoroutine(shake.ShakeCamera(0.05f, 0.1f, 0.1f));
